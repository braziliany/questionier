...
await (async () => {
  ...
  const updateScript = async (url, filePath, data) => {
    ...
    if (scriptContent) {
      // .replace(/\/\/.*\n\/\/.*\n\/\/.*\n/, '')
      const cleanedText = scriptContent.replace(/await main\([^\)]*\);?/, '').trim();
      const api = `${url}/${encodeURIComponent(name)}`;
      const { sha } = await httpRequest('GET', api, headers);
      const { content } = await httpRequest('PUT', api, headers, {
        message: `Update File:  ${filePath}/${name}`,
  ...
  const presentMenu = async () => {
    ...
    const menuList = ['GitHub 仓库', 'iCloud Files', 'Scriptable Files'];
    ...
    let path = '';
    const file = response === 1 ? await getIcloudFile() : response === 2 ? await getScript() : null;
